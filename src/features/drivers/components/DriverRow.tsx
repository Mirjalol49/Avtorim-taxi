                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-200 dark:border-gray-600 flex-shrink-0">
                        {driver.avatar ? (
                            ...
                            <div className={`w-full h-full flex items-center justify-center text-sm font-bold ${theme === 'dark' ? 'bg-[#1C1D23] text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
            ...
            <td className="p-4">
                {car ? (
                    <div className="flex items-center gap-2">
                        <div className={`w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 ${theme === 'dark' ? 'bg-[#1C1D23]' : 'bg-gray-100'}`}>
                            {car.avatar ? (
        ...
        {viewingDoc && createPortal(
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative z-10 w-full max-w-4xl h-full max-h-[85vh] flex flex-col items-center justify-center animate-in zoom-in-95 duration-300 pointer-events-none">
                    ...
                    <button 
                        onClick={() => setViewingDoc(null)} 
                        className={`absolute -top-12 right-0 md:-right-12 md:top-0 w-10 h-10 flex items-center justify-center rounded-full transition-colors bg-[#1C1D23] text-white hover:bg-white/[0.06] pointer-events-auto`}>
