...
export const DriverCard: React.FC<DriverCardProps> = ({
    ...
    return (<>
        <div className={`rounded-2xl flex flex-col transition-all group relative border overflow-hidden ${theme === 'dark'
            ...
            {/* Daily plan stats */}
            {explicitDailyPlan > 0 && (
                <div className={`mx-4 mb-3 rounded-xl p-3 flex justify-between items-center ${theme === 'dark' ? 'bg-[#1C1D23]/70 border border-white/[0.08]' : 'bg-gray-50 border border-gray-200'}`}>
            ...
            {car ? (
                <div className={`mx-4 mb-3 rounded-2xl border p-2.5 flex items-center gap-3 transition-all duration-200 ${theme === 'dark'
                    ? 'bg-[#13141A]/60 border-white/[0.06] hover:bg-[#13141A] hover:border-white/[0.08]'
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                        ...
                        <div className={`inline-flex items-center px-2 py-0.5 rounded-md border ${theme === 'dark' ? 'bg-[#0D0E12]/80 border-white/[0.08] text-gray-300' : 'bg-white border-gray-200/80 text-gray-700 shadow-sm'}`}>
                ...
                <div className={`mx-4 mb-3 rounded-2xl border p-2.5 flex items-center gap-3 ${theme === 'dark' ? 'bg-[#1C1D23]/30 border-white/[0.06] text-gray-500' : 'bg-gray-50/30 border-gray-100 text-gray-400'}`}>
            ...
            {/* Documents */}
            {docs.length > 0 && (
                <div className={`mx-4 mb-3 rounded-xl border p-3 space-y-2 ${theme === 'dark' ? 'border-white/[0.08] bg-[#1C1D23]/50' : 'border-gray-100 bg-gray-50'}`}>
        ...
        {viewingDoc && createPortal(
            ...
            >
                ...
                <div className="relative z-10 w-full max-w-5xl h-full flex flex-col items-center justify-center animate-in zoom-in-95 duration-300 pointer-events-none">
                    ...
                    {/* Floating Toolbar */}
                    <div className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 pointer-events-auto flex items-center gap-4 bg-[#0D0E12]/80 hover:bg-[#0D0E12] backdrop-blur-2xl border border-white/10 rounded-full pl-6 pr-2 py-2 shadow-2xl transition-all duration-300">
