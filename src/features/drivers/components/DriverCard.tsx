...
export const DriverCard: React.FC<DriverCardProps> = ({
    ...
    return (<>
        <div className={`rounded-2xl flex flex-col transition-all group relative border overflow-hidden ${theme === 'dark'
            ? 'bg-[#181818] border-white/[0.08] hover:border-white/[0.12]'
            ...
            <div className="flex items-center gap-4 p-5 pb-3">
                <div className={`w-16 h-16 rounded-full border-2 overflow-hidden flex-shrink-0 transition-colors ${theme === 'dark' ? 'border-gray-600 group-hover:border-[#0f766e]' : 'border-gray-200 group-hover:border-[#0f766e]'}`}>
                    {driver.avatar ? (
                        ...
                        <div className={`w-full h-full flex items-center justify-center text-xl font-bold ${theme === 'dark' ? 'bg-[#111111] text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
            ...
            {/* Daily plan stats */}
            {explicitDailyPlan > 0 && (
                <div className={`mx-4 mb-3 rounded-xl p-3 flex justify-between items-center ${theme === 'dark' ? 'bg-[#111111] border border-white/[0.08]' : 'bg-gray-50 border border-gray-200'}`}>
            ...
            {car ? (
                <div className={`mx-4 mb-3 rounded-2xl border p-2.5 flex items-center gap-3 transition-all duration-200 ${theme === 'dark'
                    ? 'bg-[#181818] border-white/[0.06] hover:bg-[#181818] hover:border-white/[0.08]'
                    : 'bg-gray-50/50 border-gray-100 hover:bg-white hover:border-gray-200 hover:shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)]'}`}>
                    
                    {/* Thumbnail Image */}
                    <div className={`relative w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden flex-shrink-0 border shadow-sm ${theme === 'dark' ? 'border-white/[0.08] bg-[#111111]' : 'border-gray-200/60 bg-gray-100'}`}>
                        {car.avatar ? (
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                        ...
                        <div className={`inline-flex items-center px-2 py-0.5 rounded-md border ${theme === 'dark' ? 'bg-[#080808] border-white/[0.08] text-gray-300' : 'bg-white border-gray-200/80 text-gray-700 shadow-sm'}`}>
                ...
                <div className={`mx-4 mb-3 rounded-2xl border p-2.5 flex items-center gap-3 ${theme === 'dark' ? 'bg-[#111111] border-white/[0.06] text-gray-500' : 'bg-gray-50/30 border-gray-100 text-gray-400'}`}>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${theme === 'dark' ? 'bg-[#111111]' : 'bg-gray-100'}`}>
            ...
            {/* Documents */}
            {docs.length > 0 && (
                <div className={`mx-4 mb-3 rounded-xl border p-3 space-y-2 ${theme === 'dark' ? 'border-white/[0.08] bg-[#111111]' : 'border-gray-100 bg-gray-50'}`}>
                            ...
                            return (
                                <button key={i} type="button"
                                    ...
                                    className={`w-full flex items-center gap-3 p-2 rounded-xl transition-all duration-200 active:scale-[0.98] outline-none focus-visible:ring-2 focus-visible:ring-[#0f766e] ${
                                        theme === 'dark' 
                                            ? 'bg-[#111111] hover:bg-white/[0.06] text-gray-200 border border-white/[0.06] hover:border-white/[0.08]' 
                                            : 'bg-white hover:bg-gray-50 text-gray-700 border border-gray-200/60 shadow-[0_1px_2px_rgba(0,0,0,0.02)] hover:shadow-md'
                                    }`}>
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${theme === 'dark' ? 'bg-[#111111] text-gray-300' : 'bg-gray-100 text-gray-500'}`}>
        ...
        {viewingDoc && typeof document !== 'undefined' && createPortal(
            ...
            >
                ...
                <div className="relative z-10 w-full max-w-5xl h-full flex flex-col items-center justify-center animate-in zoom-in-95 duration-300 pointer-events-none">
                    ...
                    {/* Floating Toolbar */}
                    <div className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 pointer-events-auto flex items-center gap-4 bg-[#080808] hover:bg-[#080808] backdrop-blur-2xl border border-white/10 rounded-full pl-6 pr-2 py-2 shadow-2xl transition-all duration-300">
