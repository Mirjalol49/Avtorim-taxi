...
export const TransactionsPage: React.FC<TransactionsPageProps> = ({
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            ...
            {/* Filters */}
            <div className={`p-4 rounded-2xl border shadow-lg ${theme === 'dark' ? 'bg-[#1C1C1E] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                                ...
                                <>
                                    ...
                                    <button
                                        ...
                                        className={`w-full px-4 py-3 rounded-xl border text-left transition-all flex items-center gap-3 ${
                                            driverModalOpen
                                                ? theme === 'dark'
                                                    ? 'bg-[#2C2C2E] border-teal-500 ring-1 ring-teal-500/40'
                                                    : 'bg-white border-teal-500 ring-1 ring-teal-500/20'
                                                : theme === 'dark'
                                                    ? 'bg-[#2C2C2E] border-white/[0.08] hover:border-white/[0.12]'
                                            ...
                                            <>
                                                <div className="w-7 h-7 rounded-lg overflow-hidden flex-shrink-0 border border-gray-600">
                                                    {selectedDriver.avatar
                                                        ...
                                                        : <div className={`w-full h-full flex items-center justify-center text-xs font-bold ${theme === 'dark' ? 'bg-[#2C2C2E] text-gray-300' : 'bg-gray-200 text-gray-600'}`}>{selectedDriver.name.charAt(0)}</div>
                                                    }
                                                </div>
            ...
            {/* Transactions Table */}
            <div className={`rounded-3xl border overflow-hidden shadow-xl ${theme === 'dark' ? 'bg-[#1C1C1E] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                        <thead className={`border-b ${theme === 'dark' ? 'bg-[#2C2C2E] border-white/[0.08]' : 'bg-gray-50 border-gray-200'}`}>
                            <tr className={`${theme === 'dark' ? 'bg-[#2C2C2E]' : 'bg-[#F2F2F7]'}`}>
                                {userRole === 'admin' && (
                                    <th className={`px-6 py-4 font-bold text-xs uppercase tracking-wider ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                                        <input
                                            ...
                                            className={`w-5 h-5 rounded-md transition-all duration-200 cursor-pointer ${theme === 'dark'
                                                ? 'bg-[#2C2C2E] border-white/[0.08] checked:bg-[#0f766e] checked:border-[#0f766e] hover:border-[#0f766e] focus:ring-2 focus:ring-[#0f766e] focus:ring-offset-0 focus:ring-offset-[#111111]'
                        ...
                        <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/[0.07]' : 'divide-black/[0.05]'}`}>
                            {paginatedTransactions.length === 0 ? (
                                ...
                                paginatedTransactions.map(tx => {
                                    ...
                                    return (
                                        <tr key={tx.id} className={`transition-colors group ${theme === 'dark' ? 'hover:bg-[#2C2C2E]' : 'hover:bg-black/[0.03]'} ${isDeleted ? 'opacity-50 grayscale' : ''}`}>
                                            {userRole === 'admin' && (
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {car ? (
                                                        <>
                                                            <div className={`w-8 h-8 rounded-full overflow-hidden border flex-shrink-0 flex items-center justify-center ${theme === 'dark' ? 'border-white/[0.08] bg-[#2C2C2E]' : 'border-gray-200 bg-gray-100'}`}>
                                                            ...
                                                            <div className="flex flex-col">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className={`text-[10px] px-1.5 py-0.5 rounded border uppercase ${theme === 'dark' ? 'border-white/[0.08] bg-[#2C2C2E] text-gray-400' : 'border-gray-200 bg-gray-100 text-gray-500'}`}>
                                                                        {t('vehicleLabel')}
                                                                    </span>
                                                                </div>
                                                        ...
                                                        <>
                                                            <div className={`w-8 h-8 rounded-full overflow-hidden border flex-shrink-0 ${theme === 'dark' ? 'border-white/[0.08]' : 'border-gray-200'} ${driver?.isDeleted ? 'opacity-50 grayscale' : ''}`}>
                                                                {driver ? <img src={driver.avatar} className="w-full h-full object-cover" alt={driver.name} /> : <div className={`w-full h-full flex items-center justify-center font-bold text-sm ${theme === 'dark' ? 'bg-[#2C2C2E] text-gray-400' : 'bg-gray-200 text-gray-500'}`}>{tx.driverName ? tx.driverName.charAt(0) : '?'}</div>}
                                                            </div>
                                                            <div className="flex flex-col">
                                                                <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            ...
                                                            {tx.type !== TransactionType.DAY_OFF && (tx.paymentMethod || tx.chequeImage) && (
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    {tx.paymentMethod && (
                                                                        <span className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border uppercase font-bold tracking-wider ${theme === 'dark' ? 'bg-[#2C2C2E] border-white/[0.08] text-gray-400' : 'bg-gray-100 border-gray-200 text-gray-500'}`}>
                ...
                {filteredTransactionsCount > itemsPerPage && (
                    <div className={`flex items-center justify-center gap-2 p-4 border-t ${theme === 'dark' ? 'border-white/[0.08] bg-[#2C2C2E]' : 'border-gray-100 bg-gray-50/50'}`}>
                        <button
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${currentPage === page
                                        ...
                                        : theme === 'dark'
                                            ? 'bg-[#2C2C2E] text-gray-400 hover:bg-white/[0.06]'
            ...
            {selectedImage && (
                ...
                >
                    ...
                    >
                        {/* Modal Header */}
                        <div className={`flex items-center justify-between px-5 py-4 border-b flex-shrink-0 ${theme === 'dark' ? 'border-white/[0.08] bg-[#2C2C2E]' : 'border-gray-100 bg-gray-50'}`}>
                            <div className="flex items-center gap-3">
