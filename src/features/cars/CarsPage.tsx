    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div className={`flex-1 p-1.5 rounded-2xl border shadow-sm ${theme === 'dark' ? 'bg-[#181818] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                    <div className="relative">
                        ...
                        <input
                            ...
                            className={`block w-full pl-10 pr-3 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0f766e] transition-colors ${theme === 'dark'
                                ? 'bg-[#111111] border-white/[0.08] text-white placeholder-gray-400'
                ...
                {userRole === 'admin' && (
                    <div className={`flex items-center p-1.5 rounded-2xl border shadow-sm ${theme === 'dark' ? 'bg-[#181818] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className={`${card} h-48 animate-pulse`}>
                            <div className={`h-full ${theme === 'dark' ? 'bg-[#111111]' : 'bg-gray-100'}`} />
                        </div>
                    ))}
                </div>
            ) : filtered.length === 0 ? (
                <div className={`text-center py-16 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${theme === 'dark' ? 'bg-[#111111]' : 'bg-gray-100'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {paginated.map(car => (
                            <div key={car.id} className={`group relative rounded-[2rem] overflow-hidden transition-all duration-300 ${theme === 'dark'
                                ? 'bg-[#181818] border border-white/[0.05] hover:border-white/[0.08] hover:shadow-2xl hover:shadow-black/50'
                                : 'bg-white border border-gray-200 hover:border-gray-300 hover:shadow-xl hover:shadow-gray-200/50'}`}>
                                
                                {/* Image Container */}
                                <div className={`relative aspect-[4/3] sm:aspect-[16/10] overflow-hidden ${theme === 'dark' ? 'bg-[#080808]' : 'bg-gray-100'}`}>
                                    {car.avatar ? (
                                        ...
                                        <div className="space-y-2">
                                            {car.documents!.map((doc, i) => (
                                                <a key={i}
                                                    ...
                                                    rel="noreferrer"
                                                    className={`group/doc flex items-center gap-4 p-3 rounded-2xl transition-all ${theme === 'dark' ? 'hover:bg-white/[0.04]' : 'hover:bg-gray-50'}`}>
                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors shadow-sm ${theme === 'dark' ? 'bg-[#111111] group-hover/doc:bg-teal-500/20' : 'bg-gray-100 group-hover/doc:bg-teal-50'}`}>
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-center gap-2 mt-6">
                            <button onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1}
                                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 ${theme === 'dark' ? 'bg-[#111111] text-white hover:bg-white/[0.06]' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50'}`}>
                                Oldingi
                            </button>
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                                <button key={p} onClick={() => setPage(p)}
                                    className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${p === page
                                        ...
                                        : theme === 'dark' ? 'bg-[#111111] text-gray-400 hover:bg-white/[0.06]' : 'bg-white text-gray-600 border border-gray-200'}`}>
                                    {p}
                                </button>
                            ))}
