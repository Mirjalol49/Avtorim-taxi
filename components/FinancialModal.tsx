...
const FinancialModal: React.FC<FinancialModalProps> = ({
  ...
  const inputClass = `w-full px-4 py-3 rounded-xl outline-none transition-all border ${isDark
    ? 'bg-[#111111] border-white/[0.08] text-white focus:border-teal-500 placeholder-gray-500'
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[99] flex items-center justify-center p-4">
      <form
        ...
        className={`flex flex-col md:flex-row w-full max-w-[1000px] max-h-[92vh] rounded-[2rem] shadow-2xl overflow-hidden border ${
          isDark ? 'bg-[#181818] border-white/[0.06]' : 'bg-white border-gray-200'
        }`}
        style={{ animation: 'modalPop 0.2s ease-out' }}
      >

        {/* ══ LEFT PANEL ══════════════════════════════════════════════════════ */}
        <div className={`flex flex-col w-full md:w-[480px] flex-shrink-0 overflow-y-auto border-b md:border-b-0 md:border-r ${isDark ? 'border-white/[0.06] bg-[#181818]' : 'border-gray-100 bg-white'}`}>

          {/* Header */}
          <div className={`sticky top-0 z-10 px-7 py-5 border-b flex items-center justify-between ${isDark ? 'border-white/[0.06] bg-[#181818]/95 backdrop-blur-md' : 'border-gray-100 bg-white/95 backdrop-blur-md'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${isDark ? 'bg-[#111111]' : 'bg-gray-100'}`}>
          ...
          <div className="p-7 space-y-6">
            ...
            {/* Type toggle */}
            <div className={`flex gap-1.5 p-1.5 rounded-2xl border ${isDark ? 'bg-[#080808] border-white/[0.08]' : 'bg-gray-100 border-gray-200'}`}>
              {[
            ...
            {type === TransactionType.EXPENSE && (
              <div className={`flex gap-1 p-1 rounded-xl border ${isDark ? 'bg-[#080808] border-white/[0.06]' : 'bg-gray-100 border-gray-200'}`}>
                {([
                  ...
                  <button key={tab.v} type="button"
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold transition-all ${
                      expenseTarget === tab.v
                        ? isDark ? 'bg-[#111111] text-white shadow' : 'bg-white text-gray-900 shadow'
              ...
              <div className="relative">
                ...
                {!isDriverOpen && selectedDriver ? (
                  <div onClick={() => setIsDriverOpen(true)}
                    className={`cursor-pointer p-4 rounded-2xl border transition-all group active:scale-[0.99] ${isDark ? 'bg-[#111111]/60 border-white/[0.08] hover:border-white/[0.12]' : 'bg-gray-50 border-gray-200 hover:border-gray-300 shadow-sm'}`}>
                    <div className="flex items-center gap-4">
                      {selectedDriver.avatar
                        ...
                        : <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-base ${isDark ? 'bg-[#111111] text-gray-300' : 'bg-gray-200 text-gray-600'}`}>{selectedDriver.name.charAt(0)}</div>
                      }
                  ...
                  <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-[#111111] border-white/[0.08] shadow-xl' : 'bg-white border-gray-200 shadow-lg'}`}>
                    <div className={`p-3 border-b ${isDark ? 'border-white/[0.08] bg-[#080808]/50' : 'border-gray-100 bg-gray-50'}`}>
                      <div className="relative">
                        ...
                        <input type="text" value={driverSearch} onChange={e => setDriverSearch(e.target.value)}
                          ...
                          className={`w-full pl-9 pr-4 py-2.5 rounded-xl outline-none text-sm ${isDark ? 'bg-[#111111] text-white placeholder-gray-500 focus:ring-2 focus:ring-teal-500/40' : 'bg-white text-gray-900 placeholder-gray-400 border border-gray-200 focus:border-teal-500'}`} />
                      </div>
                    </div>
                      ...
                      {filteredDrivers.map(d => (
                        <div key={d.id} onClick={() => { setDriverId(d.id); setIsDriverOpen(false); setDriverSearch(''); }}
                          ...
                          {d.avatar
                            ...
                            : <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold ${isDark ? 'bg-[#111111] text-gray-300' : 'bg-gray-100 text-gray-600'}`}>{d.name.charAt(0)}</div>
                          }
                          <div className="flex-1 min-w-0">
                    ...
                    {selectedDriver && (
                      <div onClick={() => setIsDriverOpen(false)}
                        className={`p-3 text-center border-t cursor-pointer text-xs font-medium transition-colors ${isDark ? 'border-white/[0.08] text-gray-500 hover:text-gray-300 hover:bg-white/[0.05]' : 'border-gray-100 text-gray-400 hover:text-gray-600 hover:bg-gray-50'}`}>
                        Yopish
                      </div>
                    )}
                  </div>
              ...
              <div className="relative">
                ...
                {!isCarOpen && selectedCar ? (
                  <div onClick={() => setIsCarOpen(true)}
                    className={`cursor-pointer p-4 rounded-2xl border transition-all group ${isDark ? 'bg-[#111111]/60 border-white/[0.08] hover:border-white/[0.12]' : 'bg-gray-50 border-gray-200 hover:border-gray-300 shadow-sm'}`}>
                    <div className="flex items-center gap-4">
                      {selectedCar.avatar
                        ? <img src={selectedCar.avatar} alt={selectedCar.name} className="w-11 h-11 rounded-xl object-cover" />
                        : <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${isDark ? 'bg-[#111111]' : 'bg-gray-200'}`}><CarIcon className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} /></div>
                      }
                    ...
                    <div className="max-h-[220px] overflow-y-auto divide-y divide-white/[0.05]">
                      {filteredCars.map(c => (
                        <div key={c.id} onClick={() => { setCarId(c.id); setIsCarOpen(false); setCarSearch(''); }}
                          ...
                          {c.avatar ? <img src={c.avatar} alt={c.name} className="w-9 h-9 rounded-lg object-cover" /> : <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${isDark ? 'bg-[#111111]' : 'bg-gray-100'}`}><CarIcon className={`w-4 h-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} /></div>}
                          <div className="flex-1 min-w-0">
                      ...
                      <button key={cat.label} type="button"
                        ...
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border text-xs font-semibold transition-all active:scale-95 ${
                          active
                            ...
                            : isDark ? 'bg-[#111111] border-white/[0.08] text-gray-400 hover:border-white/[0.12] hover:text-gray-200' : 'bg-gray-50 border-gray-200 text-gray-500 hover:border-gray-300 hover:text-gray-700'
                        }`}
                      >
        ...
        <div className={`flex flex-col flex-1 overflow-y-auto ${isDark ? 'bg-black/10' : 'bg-gray-50/60'}`}>
          ...
          <div className="px-7 pb-7 space-y-6 flex-1 -mt-2">
                    ...
                    <button key={pm.id} type="button"
                      ...
                      className={`flex flex-col items-center justify-center gap-2.5 h-[88px] rounded-2xl border text-sm font-bold transition-all active:scale-95 ${
                        paymentMethod === pm.id
                          ...
                          : isDark ? 'bg-[#111111]/60 border-white/[0.08] text-gray-400 hover:border-white/[0.12]' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-300 shadow-sm'
                      }`}
                    >
            ...
            {paymentMethod === 'card' && type !== TransactionType.DAY_OFF && (
              <div>
                ...
                {chequeImage ? (
                  <div className={`relative rounded-2xl overflow-hidden border-2 ${isDark ? 'border-teal-500/40 bg-[#080808]' : 'border-teal-300 bg-gray-50'}`}>
                  ...
                  <div onClick={() => chequeRef.current?.click()}
                    ...
                    className={`flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed cursor-pointer transition-all py-8 px-4 group ${
                      isDark ? 'border-white/[0.08] hover:border-teal-500/50 bg-[#111111]/40 hover:bg-teal-500/5' : 'border-gray-300 hover:border-teal-400 bg-white hover:bg-teal-50/50 shadow-sm'
                    }`}
                  >
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl transition-transform group-hover:-translate-y-1 ${isDark ? 'bg-[#111111]' : 'bg-gray-100'}`}>🧾</div>
                    <div className="text-center">
                      <p className={`text-sm font-bold ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Karta chekini yuklang</p>
                      <p className={`text-xs mt-1 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>
                        Drag & drop · <kbd className={`px-1.5 py-0.5 rounded text-[10px] font-mono ${isDark ? 'bg-[#111111] border border-white/[0.08]' : 'bg-gray-100 border border-gray-200'}`}>Ctrl+V</kbd> · yoki bosing
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div className={`mt-auto px-7 py-5 flex justify-end gap-3 border-t ${isDark ? 'bg-transparent border-white/[0.06]' : 'bg-gray-50 border-gray-100'}`}>
            <button type="button" onClick={resetAndClose}
              className={`px-6 py-3 rounded-xl text-sm font-bold transition-all active:scale-95 ${isDark ? 'bg-[#111111] text-gray-300 hover:bg-white/[0.06] border border-white/[0.08]' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200 shadow-sm'}`}>
              {t('cancel')}
            </button>
            <button type="submit"
