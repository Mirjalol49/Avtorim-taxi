...
import { deleteSalaryWithSync } from '../services/salaryService';
import { refundSalaryPayment } from '../services/reversalService';
import CustomSelect from './CustomSelect';
import StatusBadge from './StatusBadge';
import NumberTooltip from './NumberTooltip';
import { SearchIcon, CheckCircleIcon, ArrowLeftCircleIcon, TrashIcon, CalculatorIcon, UsersIcon } from './Icons';
...
const SalaryManagement: React.FC<SalaryManagementProps> = ({ drivers, transactions, theme, userRole, onPaySalary, salaryHistory, adminName = 'Admin', fleetId }) => {
    const { t, i18n } = useTranslation();
    const language = i18n.language as 'uz' | 'ru' | 'en'; // Helper for utils requiring language
    // Data State
    ...
    const monthOptions = useMemo(() => {
        return monthNames.map((m, i) => ({ id: i.toString(), name: m }));
    }, [monthNames]);
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    await refundSalaryPayment(
                        ...
                        t('salaryRefundDescription') || 'Salary Refund', // Pass translated description
                        fleetId
                    );
                    ...
                    addToast('error', "To'lovni qaytarishda xatolik yuz berdi");
                } finally {
    ...
    // Delete Handler
    const handleDeleteSalary = async (salary: DriverSalary) => {
        const driver = drivers.find(d => d.id === salary.driverId);
        const driverName = driver?.name || 'Unknown Driver';
        ...
        setConfirmModal({
            ...
            message: `Haqiqatan ham ushbu to'lov tarixini o'chirmoqchimisiz?\n\nHaydovchi: ${driverName}\nSumma: ${formatNumberSmart(salary.amount, false, language)} UZS\n\nBu amal to'lovni va unga tegishli xarajat tranzaksiyasini butunlay o'chirib yuboradi.`,
            isDanger: true,
            onConfirm: async () => {
                closeConfirmModal();
                try {
                    setReversingMap(prev => ({ ...prev, [salary.id]: true })); // Reuse loading state

                    await deleteSalaryWithSync(
                        salary.id,
                        salary.driverId,
                        salary.amount,
                    ...
                    setSalaryHistory(prev => prev.filter(s => s.id !== salary.id));

                    addToast('success', "To'lov tarixi va tranzaksiya o'chirildi");
                } catch (error) {
    ...
    // Stats Calculations - Re-implement based on updated variables
    ...
    const periodTransactions = useMemo(() => {
        return transactions.filter(t => {
            ...
            const isActive = t.status !== PaymentStatus.REVERSED && t.status !== PaymentStatus.REFUNDED;
    ...
    const periodPaidSalaries = salaryHistory.filter(s => {
        const d = new Date(s.effectiveDate);
        const matchesPeriod = d.getMonth() === selectedMonth && d.getFullYear() === selectedYear;
        const isActive = s.status !== PaymentStatus.REVERSED && s.status !== PaymentStatus.REFUNDED;
        return matchesPeriod && isActive;
            ...
            {viewMode === 'current' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    ...
                    <div className={`rounded-xl p-6 border ${theme === 'dark' ? 'bg-[#1E293B] border-[#334155]' : 'bg-white border-gray-200'}`}>
                        ...
                        <NumberTooltip value={periodPaidSalaries} theme={theme}>
                    ...
                    <div className={`rounded-xl p-6 border ring-1 ${pendingSalaries === 0
                        ...
                        <NumberTooltip value={pendingSalaries} theme={theme}>
                    ...
                    <div className={`rounded-xl p-6 border ${theme === 'dark' ? 'bg-[#1E293B] border-[#334155]' : 'bg-white border-gray-200'}`}>
                        ...
                        <NumberTooltip value={totalSalaries} theme={theme}>
                        ...
                        <p className={`text-xs mt-2 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
                            {monthNames[selectedMonth]} {selectedYear} {t('estimate')}
                        </p>
                    </div>
                </div>
            )}
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-16 text-center">
                                            <div className={`flex flex-col items-center justify-center ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>
                                                ...
                                                <p className="text-sm">No salaries calculated for {monthNames[selectedMonth]} {selectedYear} yet</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                                        ...
                                                        hasNoSalary ? (
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    const effectiveDate = isCurrentMonth ? now : new Date(selectedYear, selectedMonth, 1);
                                    ...
                                    (filteredData as DriverSalary[]).map((record) => {
                                        ...
                                        return (
                                            <tr key={record.id} className={`h-[72px] transition-colors ${theme === 'dark' ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'} ${isRefunded ? 'opacity-60' : ''}`}>
                                                ...
                                                <td className="px-6 py-4">
                                                    <StatusBadge status={record.status || PaymentStatus.COMPLETED} theme={theme} size="lg" showIcon={true} />
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    {isRefunded ? (
                                                        <button
                                                        ...
                                                        userRole === 'admin' && (
                                                            <div className="flex items-center justify-end gap-2">
                                                                <button
                                                                    ...
                                                                    className={`px-3 py-2 rounded-lg text-xs font-medium transition-all text-white ${reversingMap[record.id]
                                                                        ? 'bg-gray-400 cursor-not-allowed'
                                                                        : 'bg-orange-500 hover:bg-orange-600 active:scale-95 shadow-sm hover:shadow'
                                                                        }`}
                                                                    title={t('reverseAction')}
                                                                >
