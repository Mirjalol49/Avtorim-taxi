...
const DateFilter: React.FC<DateFilterProps> = ({
    ...
    return (
        <div className={`flex items-center p-1 rounded-xl w-full overflow-x-auto border backdrop-blur-sm ${theme === 'dark' ? 'bg-[#11131B] border-white/[0.08]' : 'bg-white border-gray-200'
            }`}>
            {filters.map((f) => (
                <button
                    key={f}
