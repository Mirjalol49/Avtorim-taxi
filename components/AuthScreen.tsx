        ...
        <div className={`backdrop-blur-2xl bg-slate-900/70 border ${success ? 'border-emerald-500/50 shadow-emerald-500/20' : 'border-slate-600/50 shadow-blue-500/10'} rounded-[32px] p-8 shadow-2xl relative overflow-hidden transition-all duration-500`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={success}
              className={`w-full font-bold py-4 rounded-2xl shadow-lg transform transition-all duration-300 ${
                  success 
                    ? 'bg-emerald-500 text-white scale-95' 
                    : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white hover:shadow-blue-600/30 active:scale-[0.98]'
              }`}
            >
              {success ? (
