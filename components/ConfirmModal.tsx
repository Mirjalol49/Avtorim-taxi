...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className={`rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in zoom-in-95 duration-300 border ${theme === 'dark' ? 'bg-[#11131B] border-white/[0.08]' : 'bg-white border-gray-200'
        }`}>
        <div className={`p-6 ${align === 'left' ? 'text-left' : 'text-center'}`}>
          ...
          <div className={`flex gap-3 ${align === 'left' ? 'justify-end' : 'justify-center'}`}>
            <button
              onClick={onCancel}
              className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-colors min-w-[100px] ${theme === 'dark' ? 'bg-[#181A24] text-gray-300 hover:bg-white/[0.06]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
            >
