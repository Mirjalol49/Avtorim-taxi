            ...
            <div ref={pickerRef} className="relative">
              <button
                ...
                className={`w-full rounded-xl border p-3 flex items-center gap-3 transition-colors text-left ${theme === 'dark'
                  ? 'bg-[#2C2C2E] border-white/[0.08] hover:border-[#0f766e]'
                ...
                {selectedCar ? (
                  <>
                    {selectedCar.avatar ? (
                      ...
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${theme === 'dark' ? 'bg-[#2C2C2E]' : 'bg-gray-200'}`}>
              ...
              {/* Dropdown list */}
              {carPickerOpen && (
                <div className={`absolute left-0 right-0 top-full mt-1 rounded-xl border shadow-xl z-10 overflow-hidden ${theme === 'dark' ? 'bg-[#1C1C1E] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                  {/* Search input */}
                  <div className={`p-2 border-b ${theme === 'dark' ? 'border-white/[0.08]' : 'border-gray-100'}`}>
                    <input
                      ...
                      className={`w-full px-3 py-2 rounded-lg text-sm outline-none border ${theme === 'dark'
                        ? 'bg-[#2C2C2E] border-white/[0.08] text-white placeholder-gray-500 focus:border-[#0f766e]'
                  ...
                  <div className="max-h-52 overflow-y-auto custom-scrollbar">
                    ...
                    {!carSearch && (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${!selectedCarId ? 'bg-white/20' : theme === 'dark' ? 'bg-[#2C2C2E]' : 'bg-gray-100'}`}>
                          <XIcon className="w-4 h-4" />
                        </div>
                      ...
                      filteredCars.map(car => (
                        ...
                        >
                          {car.avatar ? (
                            ...
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${selectedCarId === car.id ? 'bg-white/20' : theme === 'dark' ? 'bg-[#2C2C2E]' : 'bg-gray-100'}`}>
