...
const DriverModal: React.FC<DriverModalProps> = ({ isOpen, onClose, onSubmit, editingDriver, cars, theme }) => {
  ...
  const inputClass = `w-full px-4 py-3 rounded-xl outline-none transition-all border ${theme === 'dark'
    ? 'bg-[#181A24] border-white/[0.08] text-white focus:border-[#0f766e] placeholder-gray-500'
  ...
  const DocUploadBox = ({ category, label }: { category: DriverDocument['category']; label: string }) => {
    ...
    return (
      <div className={`rounded-xl border p-3 flex items-center gap-3 ${theme === 'dark' ? 'bg-[#181A24] border-white/[0.08]' : 'bg-gray-50 border-gray-200'}`}>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className={`rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border ${theme === 'dark' ? 'bg-[#11131B] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
        <div className={`px-6 py-5 border-b flex justify-between items-center ${theme === 'dark' ? 'border-white/[0.08] bg-[#181A24]/50' : 'border-gray-100 bg-gray-50/50'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-5">
            <div className="flex-shrink-0">
              ...
              <div className={`relative group w-20 h-20 rounded-2xl overflow-hidden border-2 cursor-pointer transition-colors ${theme === 'dark' ? 'bg-[#181A24] border-white/[0.08] hover:border-[#0f766e]' : 'bg-gray-50 border-gray-200 hover:border-[#0f766e]'}`}>
                {avatar ? (
                  <img src={avatar} alt="Preview" className="w-full h-full object-cover" />
                ) : (
                  <div className={`w-full h-full flex items-center justify-center ${theme === 'dark' ? 'bg-[#181A24]/50' : 'bg-gray-100'}`}>
            ...
            <div ref={pickerRef} className="relative">
              <button
                ...
                className={`w-full rounded-xl border p-3 flex items-center gap-3 transition-colors text-left ${theme === 'dark'
                  ? 'bg-[#181A24] border-white/[0.08] hover:border-[#0f766e]'
                ...
                {selectedCar ? (
                  <>
                    {selectedCar.avatar ? (
                      ...
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${theme === 'dark' ? 'bg-[#181A24]' : 'bg-gray-200'}`}>
              ...
              {/* Dropdown list */}
              {carPickerOpen && (
                <div className={`absolute left-0 right-0 top-full mt-1 rounded-xl border shadow-xl z-10 overflow-hidden ${theme === 'dark' ? 'bg-[#11131B] border-white/[0.08]' : 'bg-white border-gray-200'}`}>
                  {/* Search input */}
                  <div className={`p-2 border-b ${theme === 'dark' ? 'border-white/[0.08]' : 'border-gray-100'}`}>
                    <input
                      ...
                      className={`w-full px-3 py-2 rounded-lg text-sm outline-none border ${theme === 'dark'
                        ? 'bg-[#181A24] border-white/[0.08] text-white placeholder-gray-500 focus:border-[#0f766e]'
                  ...
                  <div className="max-h-52 overflow-y-auto custom-scrollbar">
                    ...
                    {!carSearch && (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${!selectedCarId ? 'bg-white/20' : theme === 'dark' ? 'bg-[#181A24]' : 'bg-gray-100'}`}>
                          <XIcon className="w-4 h-4" />
                        </div>
                      ...
                      filteredCars.map(car => (
                        ...
                        >
                          {car.avatar ? (
                            ...
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${selectedCarId === car.id ? 'bg-white/20' : theme === 'dark' ? 'bg-[#181A24]' : 'bg-gray-100'}`}>
